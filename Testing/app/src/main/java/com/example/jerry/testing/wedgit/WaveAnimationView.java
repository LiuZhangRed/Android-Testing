package com.example.jerry.testing.wedgit;
...
public class WaveAnimationView extends View {
    ...
    public WaveAnimationView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //获取自定义属性
        TypedArray typedArray = context.obtainStyledAttributes(attrs, com.handmark.pulltorefresh.library.R.styleable.WaveAnimationView);

        //波形参数初始化
        density = 1.f;
        frequency = typedArray.getFloat(com.handmark.pulltorefresh.library.R.styleable.WaveAnimationView_frequency,1.0f);
        phaseShift = -0.25f;
        ...
        //logo图片
        BitmapDrawable originalBitmap = (BitmapDrawable) typedArray.getDrawable(com.handmark.pulltorefresh.library.R.styleable.WaveAnimationView_logoImage);
